...
from src.ai_client import ClaudeClient, GeminiClient, AIClientFactory, AIClientError
...
class TestClaudeClient:
    ...
    def claude_client(self, settings):
        ...
        with patch.object(ClaudeClient, '_load_prompt_template', return_value="Test prompt: {directory_path}"):
            return ClaudeClient(settings)
    
    def test_init(self, settings):
        """Test Claude client initialization."""
        with patch.object(ClaudeClient, '_load_prompt_template', return_value="Test prompt"):
    ...
    def test_build_prompt(self, claude_client):
        ...
        claude_client.prompt_template = "Path: {directory_path}\nFiles: {file_list}\nCode: {code_snippets}\nChildren: {children_digests}"
    ...
    def test_call_claude_cli_success(self, mock_run, claude_client):
        ...
        mock_run.return_value = Mock(
            returncode=0,
            stdout='{"name": "test", "summary": "test module"}'
        )
        ...
        expected_cmd = [
            "claude",
            "--append-system-prompt", "Output JSON only",
            "--model", "claude-3-sonnet", 
            "--max-tokens", "4000",
            "--prompt", "test prompt"
        ]
    ...
    def test_analyze_directory_success(self, mock_build_prompt, mock_call_cli, claude_client):
