...
from src.analyzer import CodebaseAnalyzer, AnalysisError
...
class TestCodebaseAnalyzer:
    ...
    def test_analyze_nested_directories(self, analyzer, tmp_path):
        ...
        leaf_digest = {"name": "services", "kind": "service", "summary": "Services module"}
        ...
        def mock_aggregate(directory, child_digests, directory_info):
            return {
                "name": directory.name,
                "kind": "infra",
                "summary": f"Aggregated {directory.name} with {len(child_digests)} children"
            }
        ...
        def mock_get_children(directory, completed_digests):
            if directory == tmp_path:
                ...
                app_key = str(app_dir)
                return [completed_digests[app_key]] if app_key in completed_digests else []
            elif directory == app_dir:
                # App has services as child
                services_key = str(services_dir)
                return [completed_digests[services_key]] if services_key in completed_digests else []
            else:
        ...
        assert "Aggregated" in result["summary"] and "with 1 children" in result["summary"]
    ...
    def test_analyze_with_errors(self, analyzer, tmp_path):
        ...
        def mock_collect_info(directory):
            ...
            return {"name": directory.name, "path": str(directory), "files": [{"name": "test.py"}], "total_files": 1}
        analyzer.traverser.collect_directory_info = Mock(side_effect=mock_collect_info)
        
        # Mock AI analysis for successful root (since it has files, it's a leaf)
        analyzer.ai_client.analyze_directory = Mock(return_value={
            "name": tmp_path.name,
    ...
    def test_analyze_keyboard_interrupt(self, analyzer, tmp_path):
        ...
        analyzer.traverser.get_analysis_order = Mock(return_value=[tmp_path])
        analyzer.cache_manager.get_cached_digest = Mock(return_value=None)
        analyzer.traverser.collect_directory_info = Mock(side_effect=KeyboardInterrupt())
    ...
    def test_clear_cache(self, analyzer, tmp_path):
        ...
        analyzer.cache_manager.clear_cache.assert_called_once_with(tmp_path, recursive=True)
    ...
    def test_analyze_leaf_directory_failure(self, analyzer):
        ...
        analyzer.ai_client.analyze_directory = Mock(side_effect=AIClientError("API error"))
    ...
    def test_analyze_parent_directory(self, analyzer, tmp_path):
        ...
        result = analyzer._analyze_parent_directory(tmp_path, child_digests, directory_info)
