...
class TestDirectoryTraverser:
    ...
    def test_should_ignore_hidden_files(self, traverser):
        ...
        assert traverser._should_ignore_file(
            Path(".prettierrc.json")
        )  # Has .json extension
        assert traverser._should_ignore_file(
            Path(".gitignore")
        )  # No extension, filtered by include_extensions
        assert traverser._should_ignore_file(
            Path(".dockerignore")
        )  # No extension, filtered by include_extensions
        assert traverser._should_ignore_file(
    ...
    def test_should_ignore_hidden_files_disabled(self):
        ...
        settings = DigginSettings(
            ...
            ignore_files=[
                ...
                "*.log",
                ".DS_Store",
            ],  # Still explicitly ignore .DS_Store
            include_extensions=[".py", ".js", ".md", ".json"],
            ignore_hidden=False,  # Disabled
        )
        traverser = DirectoryTraverser(settings)
        ...
        assert not traverser._should_ignore_file(
            Path(".eslintrc.js")
        )  # Has .js extension
        assert not traverser._should_ignore_file(
            Path(".prettierrc.json")
    ...
    def test_ignore_hidden_priority(self):
        ...
        # Hidden files should be ignored due to ignore_hidden,
        ...
        assert traverser._should_ignore_file(
            Path(".config.txt")
        )  # Hidden, even with valid extension
